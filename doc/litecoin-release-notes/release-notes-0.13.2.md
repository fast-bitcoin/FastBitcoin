FastBitcoin Core version 0.13.2 is now available from:

  <https://download.fastbitcoin.cc/fastbitcoin-0.13.2.1/>
...
ECDSA signatures inside FastBitcoin transactions now use validation using
[libsecp256k1](https://github.com/fastbitcoin-core/secp256k1) instead of OpenSSL.
...
(https://github.com/fastbitcoin/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of FastBitcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of FastBitcoin Core was to raise the default minimum
relay fee.

FastBitcoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
FastBitcoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running fastbitcoind without having to do any manual configuration.
...
FastBitcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. FastBitcoin Core will relay transactions with
...
entirely in FastBitcoin Core 0.13, and it is at this time undecided whether the
...
FastBitcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
FastBitcoin Core automatically creates a hidden service to listen on, without
manual configuration. FastBitcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if FastBitcoin Core is listening, and
...
FastBitcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals FastBitcoin
Core to use floating fees. By default, FastBitcoin Core will use floating
fees.
...
(default: `0.0002` FBC/kB).

At all times, FastBitcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) FBC.
Furthermore, FastBitcoin Core will never create transactions paying less than
...
`fastbitcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of Bitcoin Core v0.11.0](https://github.com/fastbitcoin/fastbitcoin/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/fastbitcoin/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/fastbitcoin/bips/blob/master/bip-0065.mediawiki)
...
caching. A sample config for apache2 could look like:
    ...
    <Location /fastbitcoinrpc>
        ...
        # optional bypass fastbitcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< fastbitcoinrpc:<password>
    </Location>
...
Restarting fastbitcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in fastbitcoin.conf
...
fastbitcoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/fastbitcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The FastBitcoin Core code base has
...
- `fastbitcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `fastbitcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
<https://github.com/fastbitcoin/fastbitcoin/pull/7184>

BIP112 soft fork to enforce OP_CHECKSEQUENCEVERIFY
--------------------------------------------------

[BIP112][] redefines the existing OP_NOP3 as OP_CHECKSEQUENCEVERIFY (CSV)
for a new opcode in the FastBitcoin scripting system that in combination with
[BIP68][] allows execution pathways of a script to be restricted based
on the age of the output being spent.

For more information about the implementation, see
<https://github.com/fastbitcoin/fastbitcoin/pull/7524>
...
FastBitcoin transactions currently may specify a locktime indicating when
...
For more information about the implementation, see
<https://github.com/fastbitcoin/fastbitcoin/pull/6566>
...
compatible peers.
[BIP 152](https://github.com/fastbitcoin/bips/blob/master/bip-0152.mediawiki)
...
HD wallets are incompatible with older versions of FastBitcoin Core.

[Pull request](https://github.com/fastbitcoin/fastbitcoin/pull/8035/files), [BIP 32](https://github.com/fastbitcoin/bips/blob/master/bip-0032.mediawiki)
...
Witness)] (https://github.com/fastbitcoin/bips/blob/master/bip-0141.mediawiki).
...
(https://github.com/fastbitcoin/bips/blob/master/bip-0141.mediawiki) for additional
details.)
...
The two phases are distinct in the FastBitcoin-Qt GUI. During the first one,
...
- The optional new p2p message "feefilter" is implemented and the protocol
  ...
  feerate. [BIP 133](https://github.com/fastbitcoin/bips/blob/master/bip-0133.mediawiki)
...
65](https://github.com/fastbitcoin/bips/blob/master/bip-0065.mediawiki)

  - OP_NOP3 has been renamed to OP_CHECKSEQUENCEVERIFY by [BIP
112](https://github.com/fastbitcoin/bips/blob/master/bip-0112.mediawiki)
...
- Each ZMQ notification now contains an up-counting sequence number that allows
  ...
  PR [#7762](https://github.com/fastbitcoin/fastbitcoin/pull/7762).
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of fastbitcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of fastbitcoins being spent is exactly
...
- **Script versioning:** Segwit makes it easy for future soft forks to allow
  ...
  currently being researched by Bitcoin and FastBitcoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the FastBitcoin
client to FastBitcoin Core 0.13.2 to signal for segwit support. When a
...
[BIP147]: https://github.com/fastbitcoin/bips/blob/master/bip-0147.mediawiki
...
- `fastbitcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `fastbitcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
