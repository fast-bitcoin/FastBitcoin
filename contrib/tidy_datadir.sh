#!/bin/bash
# Copyright (c) 2013 The Fastbitcoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Fastbitcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Fastbitcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Fastbitcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Fastbitcoin 0.7 datadir."
    ;;
  3)
    echo "Detected Fastbitcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Fastbitcoin 0.8 datadir."
    ;;
esac
