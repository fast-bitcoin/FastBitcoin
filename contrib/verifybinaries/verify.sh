...
###   fastbitcoincore.org and fastbitcoin.cc and compares them.
...
WORKINGDIR="/tmp/fastbitcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://fastbitcoincore.org"
HOST2="https://fastbitcoin.cc"
BASEDIR="/bin/"
VERSIONPREFIX="fastbitcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'fastbitcoin-',
...
if [ $? -ne 0 ]; then
   echo "fastbitcoin.cc failed to provide signature file, but fastbitcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "fastbitcoin.cc and fastbitcoincore.org signature files were not equal?"
