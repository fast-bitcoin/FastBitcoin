...
The RPM spec file provided here is for Fastbitcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://fastbitcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Fastbitcoin git repository](https://github.com/fastbitcoin/fastbitcoin) but are not part of
...
Currently only `Source10` is used, for the example `fastbitcoin.conf` file.
...
in the fastbitcoin git repository. At present only an SVG version of the Fastbitcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the fastbitcoin
...
needed when 0.12.1 is released, a proper fix is already in the Fastbitcoin git
master branch.
...
idea to build Fastbitcoin Core against a static version of that library compiled
...
The `fastbitcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `fastbitcoin-server` package. At this point, most RPM based
...
The `fastbitcoin-server` package should have SELinux support. How to properly do
...
Fastbitcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/fastbitcoin/). He has been
packaging Fastbitcoin for Fedora at least since 2012.
...
4. I renamed the `fastbitcoin` package that contains the Qt GUI to `fastbitcoin-core` as
that appears to be how the general population refers to it, in contrast to
`fastbitcoin-xt` or `fastbitcoin-classic`. I wanted to make sure the general population
