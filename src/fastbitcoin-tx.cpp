...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || IsArgSet("-?") || IsArgSet("-h") || IsArgSet("-help"))
    {
        ...
        std::string strUsage = strprintf(_("%s fastbitcoin-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  fastbitcoin-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded fastbitcoin transaction") + "\n" +
              "  fastbitcoin-tx [options] -create [commands]   " + _("Create hex-encoded fastbitcoin transaction") + "\n" +
              "\n";
...
static void MutateTxAddOutAddr(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    CFastbitcoinAddress addr(strAddr);
    if (!addr.IsValid())
...
static void MutateTxAddOutPubKey(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    CFastbitcoinAddress addr(scriptPubKey);
    ...
    if (bScriptHash) {
        ...
        CFastbitcoinAddress redeemScriptAddr(scriptPubKey);
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded fastbitcoin transaction
