 ...
 * between fastbitcoind, and fastbitcoin-qt and non-server tools.
*/
...
void PrepareShutdown() //TODO--
{
    ...
    RenameThread("fastbitcoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), FASTBITCOIN_CONF_FILENAME));
    if (mode == HMM_FASTBITCOIND)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), FASTBITCOIN_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/Limxtec/fastbitcoin>";
    const std::string URL_WEBSITE = "<https://fastbitcoin.info>";

	//TODO-- add fastbitcoin license
    return FormatParagraph(strprintf(_("Copyright (C) 2009 -%i The Bitcoin Core developers"), COPYRIGHT_YEAR)) + "\n" +
		   FormatParagraph(strprintf(_("Copyright (C) 2014 -%i The Dash Developers"), COPYRIGHT_YEAR)) + "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015 -%i The Fastbitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" + "\n" +
...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles)
{
    ...
    RenameThread("fastbitcoin-loadblk");
...
/** Sanity checks
 *  Ensure that Fastbitcoin is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("fastbitcoin version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single fastbitcoin process is using the data directory.
    ...
    try {
       ...
       // Wait maximum 10 seconds if an old wallet is still running. Avoids lockup during restart
        if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Fastbitcoin is probably already running."), strDataDir));
        if (probeOnly) {
            lock.unlock();
        }
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(GetArg("-conf", FASTBITCOIN_CONF_FILENAME)).string());
	...
	if (mapMultiArgs.count("-masternodepaymentskey")) // masternode payments priv key
    {
        if (!masternodePayments.SetPrivKey(GetArg("-masternodepaymentskey", ""))){
			...
			//LogPrintf("Masternode: Create new key %s", CFastbitcoinSecret(secret).ToString());
   ...
   //if (!fDisableWallet) {
    ...
    fProUserModeDarksendInstantX = GetBoolArg("-promode", false); //FastBitcoindev im Standart an (Darksend und Instantx ist im QT nicht sichtbar)
	fProUserModeDarksendInstantX2 = GetBoolArg("-disable_DS_InstantX", false);  // FastBitcoindev im Standart aus (Darksend und Instantx ist im Core an)
