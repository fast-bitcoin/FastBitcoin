...
#if defined(NDEBUG)
# error "Fastbitcoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Fastbitcoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache, bool fDryRun)//TODO--
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s FBC additional fees, %d delta bytes\n",
...
void ThreadScriptCheck() {
    RenameThread("fastbitcoin-scriptch");
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if(!IsSporkActive(SPORK_1_MASTERNODE_PAYMENTS_ENFORCEMENT)){
        MasternodePayments = true; // Fastbitcoindev
    ...
    if(MasternodePayments)
    {
        ...
        if(pindex != NULL){
            if(pindex->GetBlockHash() == block.hashPrevBlock){
				...
				CAmount hardblockpowreward = block.vtx[0]->vout[0].nValue; //write by Fastbitcoindev 02-06-2015//todo++
				if(fDebug) LogPrintf("## Hardblockreward ## CheckBlock() : FBC masternode payments %d\n", hardblockpowreward);
