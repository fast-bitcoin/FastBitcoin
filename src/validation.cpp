                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s FBC additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
	...
	int FBCFullblock = (42987 + fork1)*4;
	// 42987 blocks from BTC done with 12.5 BTC, 10000 blocks from prefork with 12.5 FBC
	if(nHeight <= fork1)
	    {
        return nSubsidy;
	    }
	    ...
	    int halvings = (nHeight+FBCFullblock) / nSubsidyHalvingInterval2;
